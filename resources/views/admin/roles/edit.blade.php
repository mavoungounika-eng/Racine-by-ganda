...
<div class="max-w-3xl mx-auto">
    <div class="premium-card">
        <div class="mb-8 pb-6 border-b-2 border-slate-700">
            ...
            <p class="text-slate-400">Modifiez les informations du rôle ci-dessous</p>
        </div>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2 form-group">
                    ...
                    <textarea name="description" id="description" rows="3"
                              ...
                              class="form-control @error('description') border-red-500 @enderror">{{ old('description', $role->description) }}</textarea>
                    @error('description')
                        <p class="error-message">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="flex items-center gap-3 p-4 bg-[#160D0C]/40 rounded-xl border border-slate-700/50">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $role->is_active) ? 'checked' : '' }}
                ...
                <div class="sm:col-span-2 form-group">
                    <div class="p-4 bg-[#160D0C]/40 rounded-xl border border-slate-700/50">
                        <h3 class="text-sm font-semibold text-white mb-3">Informations</h3>
                        <dl class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                            <div>
                                <dt class="text-xs text-slate-400 mb-1">Utilisateurs associés</dt>
                                <dd class="text-lg font-bold text-racine-orange">{{ $role->users()->count() }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs text-slate-400 mb-1">Créé le</dt>
                                <dd class="text-sm text-slate-300">{{ $role->created_at->format('d/m/Y à H:i') }}</dd>
                            </div>
                        </dl>
                    </div>
                </div>
