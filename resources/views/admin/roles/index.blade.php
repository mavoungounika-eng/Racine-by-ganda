...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="premium-card">
        <form method="GET" action="{{ route('admin.roles.index') }}" class="grid md:grid-cols-3 gap-4">
            <input type="text"
                   name="search"
                   id="search"
            ...
            <div class="flex gap-2">
                <button type="submit"
                        class="flex-1 px-6 py-3 bg-gradient-to-r from-racine-orange to-racine-yellow text-white rounded-xl font-semibold hover:shadow-lg hover:shadow-racine-orange/30 transition-all flex items-center justify-center gap-2">
            ...
            <table class="premium-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <code class="px-2 py-1 bg-slate-800 text-slate-300 rounded text-xs">{{ $role->slug }}</code>
                        </td>
                        <td>
                            <div class="text-sm text-slate-400">{{ Str::limit($role->description ?? '-', 50) }}</div>
                        </td>
                        <td>
                            <div class="font-semibold text-white">{{ $role->users_count ?? 0 }}</div>
                        </td>
                        <td>
                            @if($role->is_active)
                        ...
                        <td>
                            <div class="flex justify-end gap-2">
                                ...
                                <button type="button"
                                        onclick="openDeleteModal({{ $role->id }}, '{{ addslashes($role->name) }}', {{ $role->users_count ?? 0 }})"
                    ...
                    <tr>
                        <td colspan="6" class="py-12 text-center">
                            <div class="flex flex-col items-center gap-3">
                                <i class="fas fa-user-tag text-5xl text-slate-600"></i>
                                <p class="text-slate-400 text-lg">Aucun rôle trouvé</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
                ...
                <div class="bg-[#160D0C] px-6 pt-6 pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-slate-300" id="modalMessage">
                                    Êtes-vous sûr de vouloir supprimer le rôle <strong id="roleName" class="text-white"></strong> ?
                                </p>
                                <p class="text-sm text-red-400 mt-2 hidden" id="usersWarning">
                                    ⚠️ Ce rôle est utilisé par <strong id="usersCount" class="text-white"></strong> utilisateur(s). La suppression n'est pas possible.
                                </p>
                            </div>
                        </div>
                    </div>
...
<script>
    function openDeleteModal(roleId, roleName, usersCount) {
        ...
        if (usersCount > 0) {
            document.getElementById('usersWarning').classList.remove('hidden');
            ...
            document.getElementById('usersWarning').classList.add('hidden');
