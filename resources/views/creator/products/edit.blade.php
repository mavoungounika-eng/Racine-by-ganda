...
@section('title', 'Modifier Produit - RACINE BY GANDA')
    ...
    <div class="premium-card">
        <div class="mb-8 pb-6 border-b-2 border-[#E5DDD3]">
            <h2 class="text-2xl font-bold text-[#160D0C]" style="font-family: 'Libre Baskerville', serif;">
                <i class="fas fa-edit text-[#ED5F1E] me-2"></i>
                Modifier le produit
            </h2>
            <p class="text-[#8B7355] mt-2">Modifiez les informations de votre produit</p>
        </div>
        ...
        <form method="POST" action="{{ route('creator.products.update', $product) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @if($product->main_image)
                    <div class="mb-4">
                        <p class="text-sm text-[#8B7355] mb-2">Image actuelle:</p>
                ...
                <input type="file" 
                       ...
                       accept="image/*"
                       class="form-control">
                <p class="form-help">Format: JPG, PNG (max 4MB). Laissez vide pour conserver l'image actuelle.</p>
                @error('main_image')
                    <p class="error-message">{{ $message }}</p>
                @enderror
            </div>
