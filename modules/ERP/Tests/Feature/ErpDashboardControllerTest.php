<?php
namespace Modules\ERP\Tests\Feature;
use Tests\TestCase;
use App\Models\User;
use App\Models\Product;
use App\Models\Role;
use Modules\ERP\Models\ErpSupplier;
use Modules\ERP\Models\ErpRawMaterial;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Illuminate\Support\Facades\Schema;

class ErpDashboardControllerTest extends TestCase
{
    use RefreshDatabase;
    protected User $user;

    protected function setUp(): void
    {
        parent::setUp();
        if (Schema::hasTable('roles')) {
            $role = Role::firstOrCreate(['slug' => 'staff'], ['name' => 'Staff', 'description' => 'Staff role']);
            $this->user = User::factory()->create([
                'auth_version' => 1,
                'role' => 'staff',
                'role_id' => $role->id,
                'status' => 'active',
            ]);
        } else {
            $this->user = User::factory()->create(['auth_version' => 1, 'status' => 'active']);
        }
    }

    /** @test */
    public function it_displays_dashboard_for_authorized_user()
    {
        $this->markTestSkipped('ERP web routes require real session — not compatible with actingAs(). TODO: add API endpoint for ERP dashboard.');
    }

    /** @test */
    public function it_shows_dashboard_statistics()
    {
        $this->markTestSkipped('ERP web routes require real session — not compatible with actingAs(). TODO: add API endpoint for ERP dashboard.');
    }
}
