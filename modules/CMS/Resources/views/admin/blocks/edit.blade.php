...
@section('title', 'Éditer un bloc CMS')

@section('content')
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-1">✏️ Éditer le bloc CMS</h1>
            <p class="text-muted mb-0">{{ $block->name }}</p>
        </div>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body">
            <form action="{{ route('cms.admin.blocks.update', $block) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('identifier') is-invalid @enderror" 
                                   id="identifier" name="identifier" value="{{ old('identifier', $block->identifier) }}" required>
                            @error('identifier')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('content') is-invalid @enderror" 
                                      id="content" name="content" rows="10">{{ old('content', $block->content) }}</textarea>
                            @error('content')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('type') is-invalid @enderror" 
                                        ...
                                        @foreach($types as $typeKey => $typeLabel)
                                            <option value="{{ $typeKey }}" {{ old('type', $block->type) == $typeKey ? 'selected' : '' }}>
                                                {{ $typeLabel }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('zone') is-invalid @enderror" 
                                        ...
                                        @foreach($zones as $zoneKey => $zoneLabel)
                                            <option value="{{ $zoneKey }}" {{ old('zone', $block->zone) == $zoneKey ? 'selected' : '' }}>
                                                {{ $zoneLabel }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('page_slug') is-invalid @enderror" 
                                           id="page_slug" name="page_slug" value="{{ old('page_slug', $block->page_slug) }}">
                                    @error('page_slug')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control @error('order') is-invalid @enderror" 
                                           id="order" name="order" value="{{ old('order', $block->order) }}" min="0">
                                    @error('order')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="mb-3">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="is_active" 
                                               name="is_active" value="1" {{ old('is_active', $block->is_active) ? 'checked' : '' }}>
